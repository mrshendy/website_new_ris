    ...
    try {
      ...
      if ($anchor.length) {
        $anchor.children('a').on('click', function (event) {
          ...
          // أولاً، نتأكد أن الرابط هو رابط داخلي (يحتوي على #)
          // this.hash يستخرج الجزء الذي يبدأ بـ # بأمان
          if (this.hash !== "") {

            // ثانياً، نتأكد أن العنصر المستهدف موجود فعلاً في الصفحة
            const targetElement = $(this.hash);
            if (targetElement.length) {

              // إذا كان الرابط داخليًا والعنصر موجودًا، نمنع السلوك الافتراضي وننفذ الأنيميشن
              event.preventDefault();

              const headerH = $(window).scrollTop() > 10 ? 90 : 90;
              $('html, body').stop().animate({
                scrollTop: targetElement.offset().top - headerH
              }, 200, 'easeInOutExpo');

              // الكود الخاص بتغيير الكلاسات يبقى كما هو
              const $target = $(this);
              $anchor.removeClass('current current-menu-ancestor current_page_item current-menu-parent');
              $target.parent().addClass('current');
            }
          }
          // إذا لم يكن الرابط داخليًا (لا يحتوي على #)، فلن يتم فعل أي شيء وسيسمح للرابط بالعمل بشكل طبيعي
        });
      }
    } catch (error) {
